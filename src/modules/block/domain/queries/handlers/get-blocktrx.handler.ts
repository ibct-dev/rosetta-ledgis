...
export class GetBlockTransactionHandler
    implements IQueryHandler<GetBlockTransactionQuery>
{
    constructor(
    ...
    async execute(command: GetBlockTransactionQuery) {
        const { args } = command;
        const { network_identifier, block_identifier, transaction_identifier } =
            args;
        ...
        if (
            ...
            if (!flag) {
                throw new BadRequestException(
                    "transaction hash is not exist in block"
                );
            }
            const operation = {
